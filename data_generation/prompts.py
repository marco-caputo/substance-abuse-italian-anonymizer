...
ADDITIONAL_INSTRUCTIONS = {
    "diaries_psych": """
    Please, make sure to use a variety of domain-specific terminology in the in the generated samples (e.g. psychiatric symptoms, disorders, medicines, treatments...). Feel free to add new terms that are not present in the given examples, given they remain plausible for the context of a substance abuse treatment facility. In addition, introduce variability in the entities and structure of each record. Do not include the same types of information in every entry — for example, some records may omit patient data entirely. Vary the overall layout and phrasing, especially at the beginning of the notes. Avoid starting all notes with the same pattern and do NOT use the formula "Mario, 42 anni", as provided examples do not do it. Rather, avoid referring explicitly to the patient, you can use simply verbs implying the subject. Also, do NOT put into the generated text the names of the labels themselves, labels like <PER> should not appear in the text in any form. Each note should have a distinct structure, different ordering of information, and variation in which entities appear.
    """,
    "diaries_therap": """
    Please, make sure to use a variety of specific entities in the in the generated samples while maintaining realistic. Feel free to invent stories and use imagination. In each generated sample, imagine a different person writing and try to use new terms that are not present in the given examples, given they remain plausible for the context of a report of substance abuse counselling. In addition, introduce variability in the entities and structure of each record. Do not include the same types of information in every entry — for example, some records may omit patient data entirely. Vary the overall layout and phrasing, especially at the beginning of the notes. Avoid starting all notes with the same pattern and do NOT use the formula "Mario, 42", as provided examples do not do it. To refer to the patient you can use simply verbs implying the subject, or you can use the proper name. Therapists rarely refer to the patient as "il paziente". Also, do NOT put into the generated text the names of the labels themselves, labels like <PER> should not appear in the text in any form. Each note should have a distinct structure, different ordering of information, and variation in which entities appear.
    """,
    "diaries_edu": """
    Please, make sure to use a variety of specific entities in the in the generated samples while maintaining realistic. Feel free to invent stories and use imagination. In each generated sample, imagine a different person writing and try to use new terms that are not present in the given examples, given they remain plausible for the context of a report of substance abuse counselling. In addition, introduce variability in the entities and structure of each record. Do not include the same types of information in every entry — for example, some records may omit patient data entirely. Vary the overall layout and phrasing, especially at the beginning of the notes. Avoid starting all notes with the same pattern and do NOT use the formula "Mario, 42", as provided examples do not do it. To refer to the patient you can use simply verbs implying the subject, or you can use the proper name. Educators rarely refer to the patient as "il paziente". Also, do NOT put into the generated text the names of the labels themselves, labels like <PER> should not appear in the text in any form. Each note should have a distinct structure, different ordering of information, and variation in which entities appear.
    """,
    "reports": """
    In the generated sample, imagine a different person writing and try to use new terms that are not present in the given examples.
    IMPORTANT: Please do NOT include anything else outside of the JSON in your response, no explanations, no text before or after (e.g. NO "Certainly, here is ..."). The JSON must have only the "text" field, and there use just raw text just as I did in the example above, do NOT use Markdown fences or other formatting (e.g. NO ---, ### ...), and do not highlight anything, keep it clean.
    """,
    "diaries_it": {
        "translation": """
...
\n""" + ADDITIONAL_INSTRUCTIONS["reports"]
...
PROMPT_REPORTS_2 = lambda report: """
    ...
    Respond with a single list of entities that follows exactly this format, with no text before or after the JSON, no explanations, and no Markdown code fences:
  ...
  - Avoid overusing labels, use them only when they strictly match the entity. Be precise and conservative.
...
def get_staff_diaries_prompt(samples_file: dict, train_test: str) -> str:
    # Build the few-shot prompt
    seed_examples = read_json_file(f"seed_samples/{train_test}/seed_{samples_file['filename']}_{train_test}.json")
...
def _generate_report_prompt_data(train_test: str):
    """Generates random data for report one-shot prompt generation."""
    seed_examples = read_json_file(f"seed_samples/{train_test}/seed_reports_{train_test}.json")
...
def get_report_text_prompt(train_test: str) -> str:
    return PROMPT_REPORTS_1(*_generate_report_prompt_data(train_test))
